...
# 캐주얼 응답 끝에 붙는 상태창 INFO + 추천 답변 — 대사·지문 뒤 딱 한 번. FE가 상단 패널/입력창에 렌더.
_INFO_EMIT = f"""\
[상태창 INFO — 응답 맨 끝에 딱 한 번]
위 대사·지문을 모두 마친 뒤, 대화와 분리된 줄에 정확히 `{INFO_SENTINEL}` 를 출력하고,
바로 다음 줄에 한 줄 JSON을 출력하라:
{{"place":"현재 장면의 장소","time_hint":"시간대 느낌","relation":"지금 상대와의 관계 한마디","situation":"이번 턴 상황 한 줄 요약","suggestions":["상대(유저)가 지금 할 법한 답변1","답변2","답변3"]}}
- place/time_hint/relation/situation은 모두 짧은 한국어. 이 줄은 화면 전용이라 대사로 노출되지 않는다.
- suggestions는 **상대(유저) 입장의 1인칭 짧은 대사** 3개 — 방금 네 말에 이어질 자연스러운 반응/질문. 서로 결이 다르게(예: 수긍/되물음/딴청). 각 20자 내외.
